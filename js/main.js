...
/* Генерация данных  объекта (объявления) */ // В DATA
...
/* ГЕНЕРАЦИЯ ОБЪЕКТА */ //  В МОДУЛЕ CARD
...
/* ВСТАВКА КАРТОЧКИ ОБЪЯВЛЕНИЯ */ // В CARD
...
// window.map.logoPin.addEventListener(`mousedown`, function (evt) {
//   evt.preventDefault();
//   let startCoords = {
//     x: evt.clientX,
//     y: evt.clientY
...
//     window.map.logoPin.style.top = (window.map.logoPin.offsetTop - shift.y) + `px`;
//     window.map.logoPin.style.left = (window.map.logoPin.offsetLeft - shift.x) + `px`;
//   };
